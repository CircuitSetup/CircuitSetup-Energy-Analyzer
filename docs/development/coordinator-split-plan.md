...
| `StateReducer` | `managers/state_reducer.py` | Strict dynamic `AnalyzerState` path updates, feature-result observation recording, recent observation pruning, grouped cleanup for processor/context state, runtime state refresh for metadata/latest power/alert evidence/recent activity, and context-state hydration from persisted store data |
...
| `StorePersistenceManager` | `managers/store_persistence.py` | Dirty tracking, retention pruning, gated store saves, and relearn-baseline store cleanup |
...
- `tests/test_store_persistence_manager.py`
