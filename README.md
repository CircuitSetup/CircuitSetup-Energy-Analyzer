...
Helper links explain related direct circuits without changing the measured aggregate. Qualified timing matches appear as recommendations, while the manual helper selector lists every eligible circuit from the same integration entry so a link can be changed or removed. All helper links are **Runs with this load (evidence only)**; the fixed relationship uses timing only to corroborate an estimate and does not transfer ownership, infer component state when the source is unavailable, or convert the estimate into a separately metered appliance.
