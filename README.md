...
The generated dashboard uses Home Assistant's current entity registry IDs, so renamed analyzer entities are respected. The first path remains `overview`, and the dashboard uses at most three full-width views: Home, Energy & Costs, and Insights. Home includes the appliance grid with profile icons, a labeled run timeline, and dropdown period selection for appliance energy or cost. Energy & Costs keeps the HVAC graph half-width on the left, overlays outdoor temperature on a second axis, and offers 24-hour, 7-day, and 30-day ranges. Correlated water-flow sensor history appears under Energy and costs only when an appliance has a configured flow source. Insights keeps non-graph mains status, contextual summaries, billing-cycle totals, and Expert-only diagnostics. Empty optional views and empty NILM graph placeholders are omitted.
