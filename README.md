...
The NILM workspace can also pair compatible on/off edges from an aggregate source. It plots measured real power in watts, confirms closely spaced transitions across consecutive samples, and assigns each compatible pair to at most one signature. Confirmed sessions also teach conservative duration bounds for later matching. Each review card opens on the newest complete active occurrence, marks its start and stop with dashed lines, and can move through earlier or later occurrences. Components already assigned, published, expected, or hidden stay out of Needs Review. Graph interval selections can be turned directly into appliance assignments for review.
