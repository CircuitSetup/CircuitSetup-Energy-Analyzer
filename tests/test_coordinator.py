    ...
    class FakeStates:
        def get(self, entity_id: str) -> Any:
            ...
            if isinstance(value, tuple):
                state, changed_minutes, *rest = value
                return SimpleNamespace(
                    state=str(state),
                    attributes=rest[0] if rest else {},
...
def test_coordinator_starts_rain_window_when_ambiguous_rain_becomes_dry() -> None:
    from custom_components.circuitsetup_energy_analyzer.coordinator import (
        EnergyAnalyzerCoordinator,
    )
    ...
    states["binary_sensor.rain"] = ("off", 30)
...
def test_coordinator_does_not_learn_flow_from_unconfigured_history() -> None:
    from custom_components.circuitsetup_energy_analyzer.coordinator import (
        EnergyAnalyzerCoordinator,
    )
...
def test_coordinator_treats_positive_numeric_flow_sensor_as_active() -> None:
    ...
    assert evidence["flow_sensor_active"] is True
...
def test_coordinator_uses_active_cycle_not_daily_runtime_for_water_flow() -> None:
    ...
    coordinator.state.run_cycle_runtime_seconds_by_circuit["washer"] = 120 * 60
    coordinator.state.run_cycle_evidence_by_circuit["washer"] = {
        "status": "idle",
    ...
    evidence = coordinator.state.water_flow_context_by_circuit["washer"]
    assert evidence["appliance_runtime_minutes"] == 0.0


def test_coordinator_shared_flow_uses_another_active_appliance() -> None:
    from custom_components.circuitsetup_energy_analyzer.coordinator import (
        EnergyAnalyzerCoordinator,
    )
...
def test_coordinator_exposes_processing_context_builder() -> None:
    from custom_components.circuitsetup_energy_analyzer import (
        coordinator as coordinator_module,
    )
...
def test_coordinator_exposes_store_persistence_manager() -> None:
    ...
    assert callable(coordinator.store_persistence.apply_retention)
    assert callable(coordinator.store_persistence.prune_events)
...
def test_nilm_controller_exposes_signature_review_actions() -> None:
    from custom_components.circuitsetup_energy_analyzer import (
        coordinator as coordinator_module,
    )
...
async def test_primary_signature_replacement_preserves_assignment_history() -> None:
    ...
    original_assignment = {
        ...
        "session_ids": ["session-condensate-1", "session-condensate-2"],
        "confirmed_session_ids": ["session-condensate-1"],
        "rejected_session_ids": ["session-condensate-2"],
        "label_interval_ids": ["interval-condensate-1", "interval-condensate-2"],
                ...
                "hvac_1": [
                    {
                        ...
                        "feedback_fingerprint": "condensate-fingerprint",
                        "assignment_id": assignment_id,
                    ...
                    },
                    {
                        ...
                        "feedback_fingerprint": "blower-fingerprint",
                    },
                ]
            },
            nilm_appliance_assignments_by_circuit={
                "hvac_1": [deepcopy(original_assignment)]
            },
        ),
        now_fn=lambda: datetime(2026, 6, 2, 13, 0, tzinfo=UTC),
    )
    preserved = {
        key: deepcopy(original_assignment[key])
        for key in (
            "session_ids",
    ...
    assert assignment["signature_fingerprints"] == ["blower-fingerprint"]
    assert {key: assignment[key] for key in preserved} == preserved
    assert {key: assignment[key] for key in identity} == identity


@pytest.mark.asyncio
async def test_primary_signature_replacement_rolls_back_when_save_fails() -> None:
    from custom_components.circuitsetup_energy_analyzer.coordinator import (
        EnergyAnalyzerCoordinator,
    )
            ...
            "hvac_1": [
                {
                    ...
                    "mains_circuit_id": "hvac_1",
                    "signature_fingerprints": ["condensate-fingerprint"],
                    "session_ids": ["session-condensate-1"],
                    "confirmed_session_ids": ["session-condensate-1"],
                    "rejected_session_ids": ["session-condensate-2"],
                    "label_interval_ids": ["interval-condensate-1"],
                    "lifecycle_state": "assigned",
                    "confidence": 0.91,
                    "role": "primary",
                }
    ...
    coordinator.ignored_nilm_signatures.add(("hvac_1", "signature-blower"))
    before_assignments = deepcopy(store_data.nilm_appliance_assignments_by_circuit)
    before_signatures = deepcopy(store_data.nilm_signatures)
    before_ignored = deepcopy(coordinator.ignored_nilm_signatures)
    ...
    assert store_data.nilm_appliance_assignments_by_circuit == before_assignments
    assert store_data.nilm_signatures == before_signatures
    assert coordinator.ignored_nilm_signatures == before_ignored


@pytest.mark.asyncio
async def test_session_assignment_claims_only_selected_session() -> None:
    from custom_components.circuitsetup_energy_analyzer.coordinator import (
        EnergyAnalyzerCoordinator,
    )
            ...
            nilm_session_history_by_circuit={
                "mains": [
                    ...
                    },
                    {
                        ...
                        "signature_fingerprint": "pump-fingerprint",
                        "start": "2026-06-02T12:30:00+00:00",
                        "end": "2026-06-02T12:45:00+00:00",
                    },
                ]
            },
    ...
    signature = coordinator.store_data.nilm_signatures["mains"][0]
    assert target["session_ids"] == ["session-pump"]
    assert target["signature_fingerprints"] == []
    assert session["assignment_id"] == "condensate-pump"
    assert "assignment_id" not in other_session
    assert "assignment_id" not in signature
    assert "review_state" not in signature
    assert "session-pump" not in competing["session_ids"]
    assert competing["signature_fingerprints"] == ["pump-fingerprint"]
...
async def test_session_assignment_resolves_duration_bound_close_alias() -> None:
    ...
    assert competing["signature_fingerprints"] == ["pump-fingerprint"]
    assert session["assignment_id"] == "condensate-pump"
    assert signature["assignment_id"] == "competing-load"
    assert signature["review_state"] == "assigned"
    published_signature = coordinator.data.nilm_review_by_circuit["mains"][0]
    assert published_signature["assignment_id"] == "competing-load"
...
@pytest.mark.asyncio
async def test_session_assignment_persists_one_projected_session() -> None:
    from custom_components.circuitsetup_energy_analyzer.coordinator import (
        EnergyAnalyzerCoordinator,
    )
    ...
    fingerprint = (
        "revision=2|direction=on|watts=50-100|var=0-25|va=50-100|"
    ...
    edges = [
        NilmEdge(start, 57.792, 0.0, 57.792, 0.0, "on"),
        NilmEdge(
    ...
    coordinator = EnergyAnalyzerCoordinator(
        ...
        store_data=FeatureStoreData(
            ...
            nilm_appliance_assignments_by_circuit={"mains": assignments},
        ),
        now_fn=lambda: datetime(2026, 6, 2, 13, 0, tzinfo=UTC),
    )
    ...
    projected = _nilm_workspace_sessions(
        edges,
        "mains",
    ...
    action = _nilm_session_payload_with_actions(projected[0])["actions"]["assign"]
    sibling_id = projected[1]["session_id"]

    await coordinator.async_assign_nilm_session(
        action["data"]["circuit_id"],
        action["data"]["session_id"],
        label="Condensate pump",
        signature_fingerprint=action["data"]["signature_fingerprint"],
        on_edge_id=action["data"]["on_edge_id"],
        assignment_id="condensate-pump",
    )
    ...
    history = coordinator.store_data.nilm_session_history_by_circuit["mains"]
    assert assignment["session_ids"] == [action["data"]["session_id"]]
    assert assignment["signature_fingerprints"] == []
    selected_history = next(
        session
        for session in history
        if session["session_id"] == action["data"]["session_id"]
    )
    assert selected_history["assignment_id"] == "condensate-pump"

    refreshed = nilm_workspace_payload([coordinator], circuit_id="mains")
    refreshed_sessions = {
        session["session_id"]: session for session in refreshed["sessions"]
    }
    assert refreshed_sessions[action["data"]["session_id"]]["assignment_id"] == (
        "condensate-pump"
    )
...
@pytest.mark.asyncio
async def test_session_assignment_does_not_materialize_missing_signature() -> None:
    from custom_components.circuitsetup_energy_analyzer.coordinator import (
        EnergyAnalyzerCoordinator,
    )
    from custom_components.circuitsetup_energy_analyzer.runtime_factory import (
    ...
    fingerprint = (
        "revision=2|direction=on|watts=50-100|var=0-25|va=150-200|"
        "pf=0.025-0.050|split=unknown|leg=unknown|leg_a=unknown|"
        "leg_b=unknown|balance=unknown"
    )
    ...
    coordinator = EnergyAnalyzerCoordinator(
        ...
        store_data=FeatureStoreData(
            nilm_signatures={
                "mains": [
                    {"signature_id": f"retained-{index}"}
                    for index in range(NILM_SIGNATURES_MAX_ITEMS_PER_CIRCUIT)
                ]
            },
            nilm_session_history_by_circuit={
                "mains": [
    ...
    coordinator.store_persistence.prune_nilm_history(now)

    assignment = coordinator.store_data.nilm_appliance_assignments_by_circuit[
        "mains"
    ][0]
    session = coordinator.store_data.nilm_session_history_by_circuit["mains"][0]
    assert assignment["session_ids"] == ["session-pump"]
    assert assignment["signature_fingerprints"] == []
    assert session["assignment_id"] == "condensate-pump"
    assert len(coordinator.store_data.nilm_signatures["mains"]) == (
        NILM_SIGNATURES_MAX_ITEMS_PER_CIRCUIT
    )
    assert all(
        item.get("feedback_fingerprint") != fingerprint
        for item in coordinator.store_data.nilm_signatures["mains"]
    )
...
async def test_session_assignment_ignores_merged_signature_duplicate() -> None:
        ...
        store_data=FeatureStoreData(
            nilm_signatures={
                "mains": [
                    ...
                    {
                        "signature_id": "signature-merged-pump",
                        "feedback_fingerprint": "pump-fingerprint",
                        "review_state": "merged",
                        "assignment_id": "merged-archive",
                    },
                ]
            },
            nilm_session_history_by_circuit={
    ...
    active, merged = coordinator.store_data.nilm_signatures["mains"]
    assignment = coordinator.store_data.nilm_appliance_assignments_by_circuit[
        "mains"
    ][0]
    session = coordinator.store_data.nilm_session_history_by_circuit["mains"][0]
    assert "assignment_id" not in active
    assert "review_state" not in active
    assert merged["assignment_id"] == "merged-archive"
...
@pytest.mark.asyncio
async def test_session_assignment_allows_duplicate_active_signatures() -> None:
    from custom_components.circuitsetup_energy_analyzer.coordinator import (
        EnergyAnalyzerCoordinator,
    )
    ...
    signatures = [
        {
            "signature_id": "signature-pump-a",
            "feedback_fingerprint": "pump-fingerprint",
        },
        {
            "signature_id": "signature-pump-b",
            "feedback_fingerprint": "pump-fingerprint",
        },
    ]
    coordinator = EnergyAnalyzerCoordinator(
    ...
    assert assignment["session_ids"] == ["session-pump"]
    assert assignment["signature_fingerprints"] == []
    assert all("assignment_id" not in signature for signature in signatures)
    assert all("review_state" not in signature for signature in signatures)


@pytest.mark.asyncio
async def test_session_assignment_rejects_ignored_signature_only_match() -> None:
    from custom_components.circuitsetup_energy_analyzer.coordinator import (
        EnergyAnalyzerCoordinator,
    )
    ...
    with pytest.raises(ValueError, match="Unknown or ambiguous retained signature"):
        await coordinator.async_assign_nilm_session(
            "mains",
            "session-pump",
            label="Condensate pump",
    ...
    assert "mains" not in coordinator.store_data.nilm_appliance_assignments_by_circuit
    assert "assignment_id" not in coordinator.store_data.nilm_signatures["mains"][0]


@pytest.mark.asyncio
async def test_session_assignment_rejects_pruned_ignored_signature_owner() -> None:
    from custom_components.circuitsetup_energy_analyzer.coordinator import (
        EnergyAnalyzerCoordinator,
    )

    fingerprint = "revision=2|direction=on|watts=50-100"
    coordinator = EnergyAnalyzerCoordinator(
        SimpleNamespace(data={}),
        store_data=FeatureStoreData(
    ...
    assert ignored["signature_fingerprints"] == [fingerprint]
    assert target["signature_fingerprints"] == []
    assert target["session_ids"] == []
    assert coordinator.store_data.nilm_signatures["mains"] == []


@pytest.mark.asyncio
async def test_session_assignment_rolls_back_when_save_fails() -> None:
    from custom_components.circuitsetup_energy_analyzer.coordinator import (
        EnergyAnalyzerCoordinator,
    )
    ...
    before_assignments = deepcopy(store_data.nilm_appliance_assignments_by_circuit)
    before_signatures = deepcopy(store_data.nilm_signatures)
    before_sessions = deepcopy(store_data.nilm_session_history_by_circuit)
    coordinator.ignored_nilm_signatures.add(("mains", "signature-pump"))
    coordinator.store_persistence.async_save_if_dirty = AsyncMock(
        side_effect=RuntimeError("save failed")
    )
    ...
    assert store_data.nilm_appliance_assignments_by_circuit == before_assignments
    assert store_data.nilm_signatures == before_signatures
    assert store_data.nilm_session_history_by_circuit == before_sessions
    assert ("mains", "signature-pump") in coordinator.ignored_nilm_signatures
    published_signature = coordinator.data.nilm_review_by_circuit["mains"][0]
    assert published_signature["review_state"] == "new"
    assert "assignment_id" not in published_signature


@pytest.mark.asyncio
async def test_session_rollback_serializes_signature_assignment() -> None:
    from custom_components.circuitsetup_energy_analyzer.coordinator import (
        EnergyAnalyzerCoordinator,
    )
        ...
        store_data=FeatureStoreData(
            nilm_signatures={
                "mains": [
                    ...
                    {
                        "signature_id": "signature-fan",
                        "feedback_fingerprint": "fan-fingerprint",
                    },
                ]
            },
            nilm_session_history_by_circuit={
    ...
    session_save_started = asyncio.Event()
    release_session_save = asyncio.Event()
    save_calls = 0

    async def save_if_dirty(_: datetime) -> None:
        nonlocal save_calls
        save_calls += 1
        if save_calls == 1:
            session_save_started.set()
            await release_session_save.wait()
            raise RuntimeError("save failed")

    coordinator.store_persistence.async_save_if_dirty = save_if_dirty
    session_task = asyncio.create_task(
        coordinator.async_assign_nilm_session(
            "mains",
            "session-pump",
            label="Condensate pump",
            assignment_id="condensate-pump",
    ...
    await session_save_started.wait()
    signature_task = asyncio.create_task(
        coordinator.async_assign_nilm_signature(
            "mains",
            "signature-fan",
            label="Air handler fan",
            assignment_id="air-handler-fan",
        )
    )
    await asyncio.sleep(0)
    signature_completed_during_session_save = signature_task.done()
    release_session_save.set()

    with pytest.raises(RuntimeError, match="save failed"):
        await session_task
    signature_assignment = await signature_task

    assert signature_completed_during_session_save is False
    assert signature_assignment["assignment_id"] == "air-handler-fan"
    fan_signature = coordinator.store_data.nilm_signatures["mains"][1]
    assert fan_signature["assignment_id"] == "air-handler-fan"
    fan_assignment = next(
        assignment
        for assignment in coordinator.store_data.nilm_appliance_assignments_by_circuit[
            "mains"
        ]
        if assignment["assignment_id"] == "air-handler-fan"
    )
    assert fan_assignment["signature_fingerprints"] == ["fan-fingerprint"]
    ...
    store_data = FeatureStoreData(
            ...
            "mains": [
                {
                    ...
                    "feedback_fingerprint": f"fingerprint-{index}",
                }
        ...
        nilm_session_history_by_circuit={
            "mains": [
                {
                    "session_id": f"session-{index}",
                    "signature_fingerprint": f"fingerprint-{index}",
                    "start": "2026-06-02T12:00:00+00:00",
                    "end": "2026-06-02T12:15:00+00:00",
                }
                for index in range(NILM_ASSIGNMENT_MAX_ITEMS_PER_CIRCUIT + 1)
