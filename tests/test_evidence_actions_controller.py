...
class _ActionCoordinator:
    ...
    def _alert_for_id(self, alert_id: str) -> object | None:
        return object() if alert_id == "alert-1" else None

    def _retire_alert_id(self, alert_id: str) -> None:
        self.retired_alerts.append(alert_id)
...
async def test_evidence_action_controller_pauses_and_acknowledges_alerts() -> None:
    ...
    acknowledged = await controller.async_acknowledge_alert("alert-1")

    assert "fridge" in coordinator.paused_circuits
    assert missing is False
    assert acknowledged is True
    assert coordinator.retired_alerts == ["alert-1"]
