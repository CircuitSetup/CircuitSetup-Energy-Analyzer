...
def test_recent_activity_timeline_uses_only_lifecycle_events() -> None:
    now = datetime(2026, 6, 3, 18, 0, tzinfo=UTC)
    start = CircuitEvent(
    ...
    assert summary.total_count == 2
    assert [item["event_type"] for item in summary.items] == ["stop", "start"]
