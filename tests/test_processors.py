...
def test_confirmed_complete_session_recovers_assignment_model() -> None:
    from custom_components.circuitsetup_energy_analyzer.processors.nilm_sample import (
        _recover_confirmed_assignment_models,
    )
    ...
    assignment = {
        ...
        "power_states_w": [],
        "transition_prototypes": [],
        "confirmed_session_ids": ["complete", "open"],
    }
    sessions = [
        {
            "session_id": "complete",
    ...
    assert _recover_confirmed_assignment_models([assignment], sessions, now) is True
    assert assignment["power_states_w"] == [0.0, 81.428]
    ...
    assert assignment["model_confidence"] == 0.85
    assert assignment["updated_at"] == now.isoformat()
    assert _recover_confirmed_assignment_models([assignment], sessions, now) is False
